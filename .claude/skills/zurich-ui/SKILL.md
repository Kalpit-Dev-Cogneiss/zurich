...
- **Strict black/white palette — no color accents.** Use `#fff`/`#000` (or `var(--c-white)`/`var(--c-black)`) for emphasis, and `rgba(255,255,255,X)` / `rgba(0,0,0,X)` for secondary hierarchy on dark/light sections respectively.
  - `var(--c-black)` `#000`, `var(--c-white)` `#fff`, `var(--c-gray)` `#666`, `var(--c-gray-light)` `#aaa`
  - Theme tokens: `--t-background`, `--t-text`, `--t-heading`, `--t-small`, `--t-line`. Add `.ui-dark` to flip the theme to dark.
...
- **Eyebrows/labels:** small (`1.0–1.25rem`), `letterSpacing: 0.1em`–`0.2em`, `textTransform: uppercase`, solid white/black or a muted `rgba` of the same.
...
- ✅ Keep the black-background, white-on-black monochrome aesthetic, with sentence-case headings (not uppercase).
- ✅ Use `svh`/`svw` and `clamp()` so layouts hold on mobile.
- ❌ Don't introduce a UI/CSS framework, add global CSS for one-off component styling, or reintroduce a color accent — this is a strict black/white palette.
